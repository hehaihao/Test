 ...
 */
public class ImportByKeystoreModelImpl implements ImportByKeystoreContract.IModel{

    @Override
    public Observable<?> importByKeystore(Context context,String walletName,String pwd, String keystoreStr) {
        return Observable.create((ObservableOnSubscribe<CusCreateWalletBean>) emitter -> {
            // 1 keystore
            KeysInfo keysInfoByKeyStore = WalletUtil.importWalletByKeyStore(pwd,keystoreStr);
            // 判断密码是否正确
            if (keysInfoByKeyStore.getCode() == 200) {
                dealRealm(context,keysInfoByKeyStore,walletName, pwd, emitter);
            } else {
                emitter.onError(new Exception(context.getString(R.string.import_by_keystore_psw_error)));
            }
        });
    }
     ...
     * @param emitter
     */
    private void dealRealm(Context context, KeysInfo mKeyStore, String walletName, String pwd, ObservableEmitter<CusCreateWalletBean> emitter) {
        RealmUtils.getRealm().executeTransaction(realm -> {
            ...
            if (StrUtils.isWalletAddress(walletAddress)) {
                ...
                cusCreateWalletBean.setType(2);//keystore
